...
public class ORDSTestsDay4 {
    ...
    public void verifyCountriesTest1() {
        ...
        Map<String,?> countryUS=response.jsonPath().get("items.find{it.country_id=='US'}");
       ...
       //find all country names from region 2
        ...
        for (Map.Entry<String, ?> entry : countryUS.entrySet()) {
            System.out.printf("key = %s, value = %s\n", entry.getKey(), entry.getValue());
        }
    }
   //let's find employee with highest salary, use Gpath
    @Test
    @DisplayName("Get highest salary")
    public void getEmployeeTest(){
        ...
        System.out.println(poorEmployee.get("firstname"));
    ...
    public void testSalary(){
        ...
        response.then().statusCode(200).body("items.salary",everyItem(greaterThan(0))).
        log().ifError();
